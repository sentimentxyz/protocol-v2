...
contract Pool is OwnableUpgradeable, PausableUpgradeable, ERC4626Upgradeable {
    ...
    // interest fees are accrued to the fee admin every time pool state is updated
    // a part of the interest generated by the pool is accrued as fees
    uint256 public interestFee;

    // origination fees are accrued to the fee admin on every borrow
    ...
    function accrueInterestAndFees() public {
        ...
        _accrueInterestFee(interestAccrued);
    ...
    function _accrueInterestFee(uint256 interestAccrued) internal {
        if (interestFee == 0) return;

        // [ROUND] floor fees in favor of pool lenders
        uint256 feeAssets = interestAccrued.mulDiv(interestFee, 1e18);

        // totalAssets() - feeAssets
        uint256 totalAssetExFees = IERC20(asset()).balanceOf(address(this)) + totalBorrows + interestAccrued - feeAssets;

        // [ROUND] round down in favor of pool lenders
        uint256 feeShares =
            feeAssets.mulDiv(totalSupply() + 10 ** _decimalsOffset(), totalAssetExFees + 1, Math.Rounding.Floor);

        ERC20Upgradeable._mint(FEE_ADMIN, feeShares);
    }
    ...
    function setInterestFee(uint256 _interestFee) external onlyOwner {
        interestFee = _interestFee;

        emit InterestFeeSet(_interestFee);
    }
}
