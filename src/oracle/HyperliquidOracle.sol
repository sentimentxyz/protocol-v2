...
contract HyperliquidOracle is IOracle {
    ...
    uint public immutable ASSET_INDEX;
    uint public immutable ASSET_AMT_SCALE;
    uint public immutable ASSET_PRICE_SCALE;

    error HyperLiquidOracle_InvalidAsset(address, address);

    constructor(address asset, uint assetIndex, uint assetAmtScale, uint assetPriceScale) {
        ASSET = asset;
    ...
    function getValueInEth(address asset, uint256 amt) external view returns (uint256) {
        ...
        uint assetPrice = SYSTEM_ORACLE.markPxs(ASSET_INDEX) * ASSET_PRICE_SCALE;
